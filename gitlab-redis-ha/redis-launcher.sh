...
PORTVAR="${ENV_VAR_PREFIX}_SERVICE_PORT"
HOSTVAR="${ENV_VAR_PREFIX}_SERVICE_HOST"
MASTER_LB_PORT="${!PORTVAR}"
MASTER_LB_HOST="${!HOSTVAR}"
QUORUM=${QUORUM:-2}

# If REDIS_PASS is not set and there is REDIS_PASSWORD_FILE, get the password from the file
if [ -z "$REDIS_PASS" ] && [ -f "$REDIS_PASSWORD_FILE" ]; then
  export REDIS_PASS="$(cat $REDIS_PASSWORD_FILE)"
fi
